    ...
    const [open, setOpen] = useState(false);
      ...
      const renderContent = () => {
        switch (display) {
          ...
          default:
          return (
                <main className = "flex flex-col space-y-5 bg-[#323232] min-h-screen min-w-full items-center">
                    <header className = "text-center bg-[#484848] w-full p-0.5 border-b-2 text-white text-sm lg:text-2xl border-white">
                        ...
                        <div className = "flex flex-col items-center space-y-2 overflow-y-auto h-auto lg:h-190 w-full text-white rounded px-5 py-5">
                            {orders.map((order) => (<OrderCard key = {1} orderId = {order.orderId} userEmailAddress = {order.userEmailAddress} startDate = {order.startDate} endDate = {order.endDate} status = {order.status}/>))}
                        </div>
                </main>   
            );
          }
        ...
        <main className = "flex flex-row bg-[#262626] w-full">
          <div className = "hidden lg:block flex flex-col justify-between bg-[#111111] w-45 border-r-2 border-white">
            <div className = "flex flex-col">
              ...
              <button onClick = {() => window.location.reload()} className = "w-full h-10 py-1 border-b-2 border-white text-white hover:bg-[#bfbfbf]"> Orders </button>
              ...
              {open && (
                <div className = "absolute right-3 flex flex-row space-x-2 bg-[#323232] p-4 rounded-xl w-auto border-2 border-white z-50">
                  ...
                  <div className = "flex flex-col">
                    ...
                    <button onClick = {() => window.location.reload()} className = "text-white p-1 border-t-2 border-[#484848]"> Orders </button>
